...
class DisplayGui():
    ...
    style.configure("Treeview", background="black", fieldbackground="gray", foreground="cornsilk3",
    ...
    @staticmethod
    def create_tree_view(master, tree_view_header, field_dict_object, dataframe_obj_values, anchor_tag, row_no, col_no):
        cols = [k for k in field_dict_object.keys()]
        tree = ttk.Treeview(master, columns=cols, height=2)
        tree.insert("", index="end", iid="Main", text=tree_view_header)
        # column_count = 1
        for k, v in field_dict_object.items():
            tree.heading(column=k, text=k, anchor='w')
            tree.column(column=k,width=90,minwidth=90)
            # tree.insert("Main", "end", values=(round(float(dataframe_obj_values[v][0]), 2)))
            tree.set("Main",column=k,value=round(float(dataframe_obj_values[v][0]), 2))
            # column_count = column_count + 1
            # tree.insert(k, index="end", iid="Main", text=tree_view_header,values=round(float(dataframe_obj_values[v][0]), 2))
        # round(float(dataframe_obj_values[v][0]), 2)
        # tree.insert("", index="end", iid="Main", text=tree_view_header)
        # for lbl, val in field_dict_object.items():
        #     tree.insert("Main", index="end", text=lbl)
        tree.grid(row=row_no, column=col_no, padx=25, pady=0, ipadx=20, sticky=anchor_tag)
    ...
    def disp_stock_price_detail_in_gui(stk_symbol):
        ...
        for obj in DisplayGui.stock_details_main_frame.pack_slaves():
            if obj.winfo_name() != 'stk_price_info_lbl_frame':
                stock_price_information = LabelFrame(DisplayGui.stock_details_main_frame,
        ...
        while True:
            ...
            intraday_high_low = {
                'Low': 'intraDayHighLow.min',
                'High': 'intraDayHighLow.max',
                # 'Value': 'intraDayHighLow.value'
            }

            week_high_low = {
                'Low': 'weekHighLow.min',
                'High': 'weekHighLow.max',
                # 'Value': 'weekHighLow.value'
            }
            row_cntr = 0
            col_cntr = 0
            ...
            Intraday_Week_information = LabelFrame(DisplayGui.stock_details_main_frame,
                                                   bg='black', fg='cyan', relief=RAISED,
                                                   name='stk_intraday_week_info_lbl_frame',
                                                   bd=10,
                                                   font='Helvetica 9 bold')
            Intraday_Week_information.pack(expand=False, fill=BOTH, anchor='n')

            DisplayGui.create_tree_view(Intraday_Week_information, 'INTRADAY HIGH LOW :', intraday_high_low,
                                        stk_price_detail_received, 'w', 0, 1)
            DisplayGui.create_tree_view(Intraday_Week_information, '52 WEEK HIGH LOW  :', week_high_low,
                                        stk_price_detail_received, 'e', 0, 2)
